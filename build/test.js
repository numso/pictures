...
/***/ function(module, exports, __webpack_require__) {
	...
	var svg = d3.select("#picture-box").append("svg").attr("width", size).attr("height", size);
	...
	function drawCircle(x, y, r) {
	  svg.append("circle").attr("class", "real-circle").attr("cx", x).attr("cy", y).attr("r", r).attr("fill", grey);
	...
	function drawCirclePreview(x, y, r) {
	  svg.select(".preview-circle").attr("cx", x).attr("cy", y).attr("r", r).attr("fill", grey);
	}

	function drawRect(x, y, w, h) {
	  svg.append("rect").attr("class", "real-rect").attr("x", x).attr("y", y).attr("width", w).attr("height", h).attr("fill", grey);
	...
	function drawRectPreview(x, y, w, h) {
	  svg.select(".preview-rect").attr("x", x).attr("y", y).attr("width", w).attr("height", h).attr("fill", grey);
	}

	function drawLine(x, y, x2, y2) {
	  var id = "rect-" + Math.floor(Math.random() * 99999);

	  svg.append("line").attr("class", "real-line").attr("class", id).attr("x1", x).attr("y1", y).attr("x2", x2).attr("y2", y2).attr("stroke", grey);

	  var mid = midpoint.apply(null, _argumentsToArray(arguments));

	  drawGuide(x, y);
	  drawGuide(mid.x, mid.y);
	  drawGuide(x2, y2, function (x, y) {
	    svg.select("." + id).attr("x2", x).attr("y2", y);
	  });
	}

	function drawLinePreview(x, y, x2, y2) {
	  svg.select(".preview-line").attr("x1", x).attr("y1", y).attr("x2", x2).attr("y2", y2).attr("stroke", grey);
	}

	function drawGuide(x, y, update) {
	  var id = "guide-" + Math.floor(Math.random() * 99999);
	  svg.append("circle").attr("class", "guide").attr("class", id).attr("cx", x).attr("cy", y).attr("r", 5).attr("fill", blue).on("mousemove", function (e) {
	    // set selected
	    // then, on mouse move, set it if selected
	    update && update(d3.event.offsetX, d3.event.offsetY);
	    svg.select("." + id).attr("cx", d3.event.offsetX).attr("cy", d3.event.offsetY);
	    // redraw guides...
	  });
	}
